...
export const expenseLines = pgTable(
  "expense_lines",
  {
    ...
    status: text("status").notNull().default("posted"),
    // PAYROLL-1 (migration 0170): when an expense line is generated by a
    // monthly payroll run it points back to the run + staff member. Both
    // nullable — manual expenses (the common case) leave them NULL. FK target
    // tables live in schema/payroll.ts; we keep these as bare uuid columns here
    // to avoid an import cycle (finance ⇄ payroll) — the DB-level FK exists.
