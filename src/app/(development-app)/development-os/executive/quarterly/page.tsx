...
export const metadata: Metadata = {
  title: "Executive · Quarterly · Development OS",
};
...
export default async function ExecutiveQuarterlyPage() {
  const db = getDb();
  if (!db) {
    return (
      <DevelopmentShell>
        <RangeHeader title="Executive" emphasis="· quarterly" sub="" />
        <EmptyState
  ...
  const [snapshotRows, alerts, digests] = await Promise.all([
    safeQuery(
      "listCompanySnapshotsInRange:quarter",
      listCompanySnapshotsInRange(start, end),
      [],
    ),
    safeQuery("listAlertsInRange:quarter", listAlertsInRange(start, end), []),
    safeQuery(
      "listDigestsInRange:quarter",
      listDigestsInRange(start, end),
      [],
    ),
  ]);
  ...
  const cashDeltaCtx =
    rollup.cashDeltaMinor == null
      ? "history accruing"
      : `${rollup.cashDeltaMinor >= 0 ? "↑" : "↓"} ${fmt(Math.abs(rollup.cashDeltaMinor))} across ${label}`;
  ...
  const sub = [
    ...
    `${rollup.risk.total} risk${rollup.risk.total === 1 ? "" : "s"} raised`,
  ].join(" · ");
  ...
  return (
    <DevelopmentShell>
      ...
      {/* Quarterly headline KPIs — variant B of the exec mock. */}
      <div className="grid grid-cols-1 gap-3.5 sm:grid-cols-2 lg:grid-cols-4">
        <RollupKpi
          label={`${label} cashflow`}
        ...
        <RollupKpi
          label="risk count"
          value={`${rollup.risk.total}`}
          ctx={`${rollup.risk.high} H · ${rollup.risk.medium} M · ${rollup.risk.low} L`}
          tone={rollup.risk.high > 0 ? "warn" : "ink"}
        />
      </div>
      ...
      <div className="grid grid-cols-1 gap-x-8 gap-y-10 lg:grid-cols-2 lg:divide-x lg:divide-line-soft">
        ...
        <section className="lg:pr-8">
          <RollupSectionHeading accent={label}>Quarter ledger</RollupSectionHeading>
          <div className="flex flex-col gap-2">
            ...
            />
            <LedgerRow
              label="Cash movement in quarter"
              value={fmt(rollup.cashDeltaMinor)}
              tone={cashDeltaTone}
            />
            <LedgerRow
          ...
          {rollup.snapshotCount > 0 ? (
            <div className="mt-5 grid grid-cols-3 gap-2.5">
              <MiniStat
                label="on track"
                value={`${rollup.projectsOnTrack ?? "—"}`}
              />
              <MiniStat
                label="at risk"
                value={`${rollup.projectsAtRisk ?? "—"}`}
              />
              <MiniStat
                label="delayed"
                value={`${rollup.projectsDelayed ?? "—"}`}
              />
            </div>
          ) : null}
        </section>

        {/* Right — risks raised + digests covering the quarter. */}
        <section className="lg:pl-8">
          ...
          <p className="mt-2.5 font-mono text-[10.5px] text-ink-tertiary">
            Counts reflect alerts detected within {label}.{" "}
            <Link
          ...
          {digests.length === 0 ? (
            <p className="rounded-md border border-line-soft bg-surface px-3.5 py-[11px] text-xs text-ink-tertiary">
              No digests cover this quarter yet — the monthly digest job
              drafts one per month.
            </p>
          ) : (
            <div className="flex flex-col gap-2">
              {digests.map((d) => (
