...
import { addPurchaseOrderLineAction } from "@/features/procurement/actions";
...
/**
 * Append a line to a purchase order. Backed by `addPurchaseOrderLineAction`
 * (perm `procurement.write`, org-scoped) which also recomputes the order's
 * total. Follows the Modal-First contract (`onSuccess`/`onCancel`).
 */
      ...
      >
        ...
        {items.length > 0 && (
          <Field
            ...
            error={errs.itemId?.[0]}
            hint="Optional — link to a catalogued SKU so receiving moves stock."
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <Field label="Quantity ordered" required error={errs.quantityOrdered?.[0]}>
            <input
