    ...
    <div className="animate-pulse" aria-busy="true" aria-label="Loading">
      ...
      <div className="flex items-end justify-between gap-4 flex-wrap">
        <div className="flex-1 min-w-0">
          <div className="h-3 w-40 rounded bg-line-soft mb-3" />
          <div className="h-8 w-64 max-w-[70%] rounded bg-muted mb-2.5" />
          <div className="h-3 w-[min(560px,90%)] rounded bg-line-soft" />
        </div>
        <div className="flex gap-2">
          <div className="h-8 w-28 rounded-full bg-line-soft" />
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 mt-[18px] mb-[18px]">
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i} className="card p-4">
            <div className="h-2.5 w-20 rounded bg-line-soft mb-3" />
            <div className="h-6 w-16 rounded bg-muted mb-2" />
            <div className="h-2.5 w-24 rounded bg-line-soft" />
          </div>
        ))}
      </div>
