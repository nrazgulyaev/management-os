# Owner Portal · Inbox — pixel build prompt
...
## Source of truth
- **Mockup (pixel target):** `cabinets/owner-p1/05-inbox.html`
...
**Repo status:** ✅ `/owner/inbox`. `owner_threads`/`owner_messages` tables exist (drizzle 0114). Redesign + wire.
...
`ThreadList` + `ThreadView` + `MsgBubble` · `owner-concierge` agent.

## Gotchas
- Use `owner_threads`/`owner_messages` tables.
- Agent = owner-concierge.
