# Development OS · Dev Workspace — pixel build prompt
...
## Source of truth
- **Mockup (pixel target):** `cabinets/new/dev-workspace.html`
...
**Repo status:** ✅ `/development-os` `page.tsx` 12.9kb. Redesign of the dev command center.
...
Bento of `<Kpi>` + `.card`/`.panel` tiles · `.card-inverted` carbon hero.

## Gotchas
- Dev cards use `--panel` + `--r-card-lg`(18) + `--line` — not the mgmt card recipe.
