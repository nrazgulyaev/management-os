# Development OS · Sales — pixel build prompt
...
## Source of truth
- **Mockup (pixel target):** `cabinets/dev-p2/sales.html`
...
- `/development-os/sales/[contactRoleId]`
- `/development-os/buyers`
- `/development-os/contracts`
- `/development-os/discounts`

**Repo status:** ✅ built — `/development-os/sales` + buyers/contracts/discounts/marketing/communications. Redesign.
...
**`PipelineBoard`** for the 6-stage pipeline FSM · `BuyerDetail`/`ContractPage`/`PaymentLadder` · offer = `OfferModal` · `FunnelChart`.

## Gotchas
- 6-stage pipeline FSM — each stage column matches the mockup order/labels.
- Offer policy gates which offers are allowed — preserve.
...
- [ ] PipelineBoard stages match mockup
- [ ] PaymentLadder matches mockup
